...
void main() {
  ...
  setUp(() {
    ...
    api = LocalStorageTodosApi(mockPrefs);
  });

  group('getTodoList', () {
    test('getTodoList returns empty list when SharedPreferences is empty',
        () async {
      when(mockPrefs.getString('todos')).thenReturn(null);
      when(mockPrefs.getInt('revision')).thenReturn(null);
    ...
    test('getTodoList returns todos when SharedPreferences contains todos',
        () async {
      when(mockPrefs.getString('todos')).thenReturn(
      ...
      when(mockPrefs.getInt('revision')).thenReturn(1);
